    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                ...
                case sf::Keyboard::W: // เดินขึ้น
                    if (gameMap[player.x + (player.y - 1) * 40] != 1) player.y -= 1;
                    break;
                case sf::Keyboard::S: // เดินลง
                    if (gameMap[player.x + (player.y + 1) * 40] != 1) player.y += 1;
                    break;
                case sf::Keyboard::D: // เดินขวา
                    if (gameMap[(player.x + 1) + player.y * 40] != 1) player.x += 1;
                    break;
                case sf::Keyboard::A: // เดินซ้าย
                    if (gameMap[(player.x - 1) + player.y * 40] != 1) player.x -= 1;
                    break;
                }
            }
